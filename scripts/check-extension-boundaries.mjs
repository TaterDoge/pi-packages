...
const activePackages = findActiveExtensionPackages(packagesDirectory);
const activePackageNames = new Set(activePackages.map((extensionPackage) => extensionPackage.name));
const failures = [];
...
function isForbiddenExtensionReference(packageName, specifier) {
  const referencedPackageName = packageNameFromSpecifier(specifier);
  return referencedPackageName !== packageName && activePackageNames.has(referencedPackageName);
}
